...
高野原店のFAXなど、数量が文字で分かっている場合はテキストでも渡せます。
**FAX注文表の読み取りは実物を受領してから対応します。**
...
- [ ] **FAXの読み取り精度を上げる**（`src/fmbill/faxreader.py`・開発中）
      傾き補正と行の切り出しは動くが、発注数の欄の位置合わせが不十分。
      FAXの縦罫線がかすれて消えるため列を自動検出できていない。
      **数量を取り違えると請求金額が狂うので、納品書の作成には繋いでいません。**
- [ ] スマホから画像を送って納品書まで作る流れ（FAX読み取りの精度が出てから）
