"""FAXで返ってきた発注書から「発注数」欄を読み取る。【開発中・未接続】

⚠️ このモジュールはまだ納品書の作成に使っていません。
   数量を取り違えると請求金額が狂うため、精度が出るまで意図的に繋いでいません。

現状:
  ✅ 傾き補正と、横罫線からの行の切り出しは実用になる精度で動く
  ❌ 発注数の欄の位置合わせが不十分（印字された「最小発注ロット」を拾ってしまう）
     FAXの縦罫線がかすれて消えるため、列の自動検出ができていない
